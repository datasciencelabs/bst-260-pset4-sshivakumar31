...
cases_raw <- request(api) |> req_perform() |> resp_body_string() |> fromJSON()
...
cases_raw |> 
  ...
  filter(!is.na(state),!is.na(date),!is.na(cases),!is.na(population),!is.na(region_name)) |>
...
cases_raw |> 
  filter(year(date) == 2020 | year(date) == 2021) |> 
  mutate(month = month(date, label = TRUE), day = day(date), year = year(date)) |>
  group_by(year, month) |>
  summarise(
